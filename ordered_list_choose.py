...
# Display the ordered_list_choose page
    ...
    with st.container():
        ...
        def handle_confirm_click():
            ...
            if any(s in placeholders for s in selections):
                st.session_state.error_msg = "select a song for each place"
            elif len(set(selections)) < 3:
                st.session_state.error_msg = "each spot needs a unique song"
            else:
        ...
        # Ordered list selection with error handling
