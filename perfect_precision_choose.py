...
# Display the perfect_precision_choose page
def perfect_precision_choose_page():
    ...
    # Prepare song list for recommendation
    if "songs_data" not in st.session_state:
        ...
        col_sorted = (st.session_state.songs_data['perfect_precision_songs'].sort_values(key=lambda c: c.str.lower()).to_numpy())
        st.session_state.songs_data['perfect_precision_songs'] = col_sorted
    ...
    with st.container():
        ...
        # Validate user ranking and move to comparison if valid
        ...
        # Perfect precision selection with error handling
        col_next = st.columns([0.15, 0.7, 0.15])
        with col_next[1]:
            selected_songs = st.multiselect("", st.session_state.songs_data["perfect_precision_songs"].tolist(), max_selections=3)
    ...
    # Display songs for user listening before selection
